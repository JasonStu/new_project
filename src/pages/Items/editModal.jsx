...
const EditModal = (props) => {
  ...
  const validateFileList = (rule, value, cb, list) => {
    ...
    return rule.required && list.length
      ? Promise.resolve()
      : Promise.reject(`${rule.field} is required!!!`);
  };
  const fileHandleMethod = (item) => {
    if (item.originFileObj) {
      ...
      let arr =
        (item && item.url && item.url.length > 0 && item.url.split("/")) || [];
          ...
          rules={[
            {
              ...
              validator: (rule, value, cb) =>
                validateFileList(rule, value, cb, fileList)
            }
          ]}
          fieldProps={{
            fileList: fileList.map((item) => fileHandleMethod(item)),
            onPreview: (f) => {
              console.log("f===", f);
            },
            onChange: (v) => {
          ...
          rules={[
            {
              required: true,
              validator: (rule, value, cb) =>
                validateFileList(rule, value, cb, fileList2)
            }
          ]}
          label="Closed Item"
          name="closed_image"
          ...
          rules={[
            {
              required: true,
              validator: (rule, value, cb) =>
                validateFileList(rule, value, cb, fileList3)
            }
          ]}
        />
      }
          ...
          rules={[
            {
              required: true,
              validator: (rule, value, cb) =>
                validateFileList(rule, value, cb, fileList4)
            }
          ]}
        />
      }
